...
def get_activation(outputs, mode):
    ...
    if mode=='avg':
        def hook(model, input, output):
            if len(output.pooler_output.shape)==4:
                #outputs.append(output.mean(dim=[2,3]).detach().cpu())
                outputs.append(output.pooler_output.detach().cpu())
            elif len(output.pooler_output.shape)==2:
                outputs.append(output.pooler_output.detach().cpu())
    elif mode=='max':
        def hook(model, input, output):
            if len(output.pooler_output.shape)==4:
...
def plot(train_losses, val_losses, accuracy, dir, name):
    try:
        t = range(len(train_losses))
        _, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))

        ax1.plot(t, train_losses, label="Train")
        ax1.plot(t, val_losses, label="Validation")
        ...
        ax2.plot(t, accuracy, color='green')
...
class FocalLoss(nn.Module):
    def __init__(self, beta: float, gamma: float = 2):
        super().__init__()
        self.b = beta
        self.r = gamma
    ...
    def forward(self, pred, y):
        ...
        p = torch.where(y == 1, pred, 1 - pred)
        fl = self.b * (1 - p) ** self.r * bce + (1 - self.b) * bce
