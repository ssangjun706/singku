...
def elastic_loss_and_acc(linear, X, y, lam, alpha, class_weight, family='multinomial'): 
    weight, bias = list(linear.parameters())
    l1 = lam * alpha * weight.norm(p=1)
    l2 = 0.5 * lam * (1 - alpha) * (weight**2).sum()
    outputs = linear(X)
    if family == 'multinomial': 
        l = F.cross_entropy(outputs,y,weight=class_weight, reduction='mean')
        acc = (outputs.max(1)[1] == y).float().mean()
    elif family == 'gaussian':
        l = 0.5*F.mse_loss(outputs, y,weight=class_weight, reduction='mean')
...
# Elastic net loss given a loader instead
def elastic_loss_and_acc_loader(linear, loader, lam, alpha, class_weight, preprocess=None, family='multinomial'): 
    loss = 0
    acc=0
    n = 0
    device = linear.weight.device
    ...
    for batch in loader: 
        ...
        if preprocess is not None: 
            X = preprocess(X)
        bs = X.size(0)
        l,a = elastic_loss_and_acc(linear, X, y, lam, alpha, class_weight, family=family)
        loss += l*bs
        acc += a*bs
        n += bs
    return loss/n, acc/n
...
def train_saga(linear, loader, lr, nepochs, lam, alpha, class_weight, group=True, verbose=None, 
    ...
    with ch.no_grad(): 
        ...
        for t in tqdm(range(nepochs)): 
            ...
            for batch in loader: 
                ...
                if family == 'multinomial': 
                    if w is None: 
                        loss = F.cross_entropy(out,y.to(weight.device), weight=class_weight, reduction='mean')
                    else: 
                        loss = F.cross_entropy(out,y.to(weight.device), weight=class_weight, reduction='none')
                        loss = (loss*w).mean()
...
def glm_saga(linear, loader, max_lr, nepochs, alpha, weight,
    ...
    for i,(lam,lr) in enumerate(zip(lams,lrs)): 
        ...
        state = train_saga(linear, loader, lr, nepochs, lam, alpha, weight,
        ...
        with ch.no_grad(): 
            loss,acc = elastic_loss_and_acc_loader(linear, loader, lam, alpha,weight, preprocess=preprocess, family=family)
            loss,acc = loss.item(),acc.item()

            loss_val,acc_val = -1,-1
            if val_loader: 
                loss_val,acc_val = elastic_loss_and_acc_loader(linear, val_loader, lam, alpha,weight, preprocess=preprocess, family=family)
                loss_val,acc_val = loss_val.item(),acc_val.item()

            loss_test,acc_test = -1,-1
            if test_loader: 
                loss_test,acc_test = elastic_loss_and_acc_loader(linear, test_loader, lam, alpha,weight, preprocess=preprocess, family=family)
